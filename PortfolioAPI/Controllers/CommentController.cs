...
public class CommentController: ControllerBase
{
    ...
    [HttpPut("add/{id}")]
    [Authorize]
    public async Task<ActionResult<Comment>> PutAddComment([FromRoute] int id)
    {
    ...
    [HttpPut("delete/{id}")]
    [Authorize]
    public async Task<ActionResult<Comment>> PutDeleteComment([FromRoute] int id)
    {
        // Get the authenticated user's username
        var userName = User.FindFirstValue(ClaimTypes.Name);
    ...
    public async Task<ActionResult<Comment>> PostComment([FromBody] string comment)
    {
        ...
        commentAwaitingCreation.Text = comment;
        commentAwaitingCreation.PostId = 0;
        commentAwaitingCreation.UserName = userName;  // Could use the username from token
